...
function render(){
  ...
  for(const e of day.exercises){
    ...
    if(e.hold){const hold=document.createElement('button');hold.className='timer-btn hold-btn';hold.textContent='⏱ Tenuta · cronometro';hold.onclick=()=>startHold(e);actions.append(hold)}
...
function openClock(){ $('timerOverlay').classList.add('show');wake();refreshClock();state.timerId=setInterval(refreshClock,150)}
function startHold(e){closeTimer();state.mode='hold';state.exerciseName=e.name;state.holdTarget=e.hold;state.restSeconds=e.rest;state.startAt=performance.now();state.elapsedBefore=0;openClock()}
...
function refreshClock(){
  ...
  if(state.mode==='hold'){
    ...
    $('timerBig').textContent=fmt(Math.max(0,elapsed));$('timerStatus').textContent='Premi «Fine tenuta» quando termini';
...
function add30(){if(state.mode==='rest'){state.deadline+=30000;refreshClock();if(state.timerId===null)state.timerId=setInterval(refreshClock,150)}}
...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 document.addEventListener('visibilitychange',()=>{if(!document.hidden&&state.mode&&state.timerId===null&&state.mode==='hold'){state.timerId=setInterval(refreshClock,150)}if(!document.hidden)refreshClock()});
